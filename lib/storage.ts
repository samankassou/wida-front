...
const draftKey = (id: string) => `wida:draft:v1:${id}`;
...
export function loadDraft(id: string, mode: WorkspaceMode): ReviewDraft | null {
  try {
    const value = JSON.parse(draftStore(mode).getItem(draftKey(id)) || "null");
...
export function saveDraft(id: string, mode: WorkspaceMode, draft: ReviewDraft) {
  try { draftStore(mode).setItem(draftKey(id), JSON.stringify(draft)); return true; } catch { return false; }
}

export function removeDraft(id: string, mode: WorkspaceMode) {
  try { draftStore(mode).removeItem(draftKey(id)); } catch { /* In-memory state remains usable. */ }
}
