...
The API extracts eight header fields and line items from the first analyzed document. Extracted lines prefill the editable form; low or unknown confidence requires an explicit check before saving. Saved invoices and edited drafts retain their values. Live PDFs use a page-by-page canvas preview, with an open-original link for browser-native text selection, printing, and unsupported or password-protected files. Selecting an extracted header or unsaved extracted line-item field highlights its first source region on uploaded PDFs and supported images, and PDFs navigate to that page. PDF, image, and sample previews share zoom and rotation controls. PDF.js is loaded on demand and its worker/assets are served locally. Older extraction runs with polygon-only geometry and manually entered fields display a no-location message; new analyses include page dimensions. Saved line items are not linked by row position, since edits may have changed their order. TIFF preview support also depends on the browser.
