...
export default function AnalysisActivity({ items, activeItems, recentUpdates, statusUnavailable, onOpen, onDismiss }: Props) {
  ...
  return <section className="processing-activity" aria-label="Analysis activity">
        ...
        {statusUnavailable && activeItems.length > 0 ? <p className="processing-connection" role="status">Progress is temporarily unavailable. We’ll check again automatically; no need to upload your documents again.</p> : null}
