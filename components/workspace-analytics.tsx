...
export default function WorkspaceAnalytics({ items, summary, onFilter }: { items: WorkspaceItem[]; summary?: WorkspaceSummary; onFilter: (stage: DocumentStage) => void }) {
  ...
  const years = summary ? [...new Set(summary.months.map(bucket => bucket.year))].sort((a, b) => b - a) : [...new Set(items.map(item => new Date(item.document.uploadedAt).getUTCFullYear()).filter(Number.isFinite))].sort((a, b) => b - a);
  const year = chosenYear ?? years[0] ?? new Date().getUTCFullYear();
  const buckets = months.map(() => ({ uploaded: 0, saved: 0 }));
  if (summary) {
    for (const bucket of summary.months) if (bucket.year === year) buckets[bucket.month - 1] = { uploaded: bucket.uploaded, saved: bucket.saved };
  ...
  const saved = summary?.counts.saved ?? items.filter(item => stageOf(item) === "saved").length;
  const percentage = total ? Math.round(saved / total * 100) : 0;
  ...
  return <section className="workspace-analytics" aria-label={t("Document analytics")}>
    ...
    <div className="health-card"><div className="analytics-heading"><h2>{t("Workspace summary")}</h2><FileCheck2 size={16} /></div><div className="health-total">{percentage}<span>%</span></div><p className="health-description">{t("of your documents saved as invoices")}</p><div className="health-progress">{statuses.map(({ stage, label, Icon }) => {
      const count = summary?.counts[stage] ?? items.filter(item => stageOf(item) === stage).length;
