...
type Turnstile = { render: (node: HTMLElement, options: { sitekey: string; action: string; callback: (token: string) => void; "error-callback": () => void }) => string; remove: (id: string) => void };
...
export default function TrialCaptcha({ siteKey, session }: { siteKey: string; session?: ApiSession }) {
  ...
  return <div><p role="status">{status}</p><div ref={node} /><Script src="https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit" onReady={render} onError={() => setStatus("Vérification indisponible. Rechargez la page pour réessayer.")} /></div>;
}
