...
function InvoiceField({ name, value, type = "text", required, placeholder, wide, extracted, checked, error, showError, onChange, onCheck, onFocus, onBlur }: FieldProps) {
  ...
  return (
    <div className={`rv-field ${wide ? "rv-field-wide" : ""} ${needsCheck && !checked ? "rv-field-review" : ""} ${invalid ? "rv-field-invalid" : ""}`}>
      <div className="rv-field-label"><label htmlFor={id}>{fieldLabels[name]}{required ? <span aria-hidden="true"> *</span> : null}</label>{needsCheck ? <span className={`rv-confidence ${checked ? "" : "rv-confidence-low"}`}>{checked ? <Check size={12} aria-hidden="true" /> : <CircleAlert size={12} aria-hidden="true" />}{checked ? "Checked" : "Check value"}</span> : null}</div>
...
export default function InvoiceReview({ item, draft, onDraftChange, onSave, onBack, onNext, hasNext, saving, mode, sourceUrl, runs, onAnalyze, analyzing, serverErrors, error }: Props) {
  ...
  function renderLineField(line: InvoiceLineDraft, index: number, key: Exclude<keyof InvoiceLineDraft, "id">, label: string, wide = false) {
    ...
    return <div className={`rv-field ${wide ? "rv-field-wide" : ""} ${needsCheck && !checked ? "rv-field-review" : ""}`} key={key}>
      <div className="rv-field-label"><label htmlFor={`rv-field-${fieldKey}`}>{label}</label>{needsCheck ? <span className={`rv-confidence ${checked ? "" : "rv-confidence-low"}`}>{checked ? "Checked" : "Check value"}</span> : null}</div>
      ...
      {key === "lineAmount" && taxInclusiveLineNet(line) !== null ? <p className="rv-confidence-note">Includes tax · Amount before tax: {taxInclusiveLineNet(line)!.toFixed(2)}.</p> : null}
  ...
  return (
    <div className="rv-workspace" ref={root}>
      ...
      <div className="rv-mobile-tabs" aria-label="Review panels"><button type="button" aria-pressed={mobilePanel === "document"} onClick={() => setMobilePanel("document")}><FileText size={15} />Original document</button><button type="button" aria-pressed={mobilePanel === "fields"} onClick={() => setMobilePanel("fields")}><Layers3 size={15} />Invoice details{issueKeys.length ? <span>{issueKeys.length}</span> : null}</button></div>
      ...
      <div className={`rv-split rv-mobile-${mobilePanel}`}>
        ...
        <section className="rv-data-panel" aria-label="Invoice review form">
          <div className="rv-data-panel-header"><div><h2>Invoice details</h2><p>{item.invoice ? "Saved record · edit to make a correction" : "Check the details before saving"}</p></div><span className="rv-extraction-icon"><ShieldCheck size={20} aria-hidden="true" /></span></div>
            ...
            <fieldset disabled={saving} className="rv-fieldset">
              <div className="rv-form-content">
                ...
                <section role="tabpanel" id="rv-panel-details" aria-labelledby="rv-tab-details" hidden={tab !== "details"}>
                  {analyzing && !dirty && !item.invoice ? null : issueKeys.length ? <div className="rv-issue-banner"><div className="rv-issue-banner-icon"><CircleAlert size={17} /></div><div><strong>{issueKeys.length} {issueKeys.length === 1 ? "item needs" : "items need"} your attention</strong><p>Compare flagged values with the original.</p></div><button type="button" onClick={focusNextIssue} aria-label="Go to next issue"><ArrowRight size={17} /></button></div> : <div className="rv-checked-banner"><CheckCheck size={17} /><span>{item.invoice && !dirty ? "Invoice details saved" : "All checks complete. Ready to save."}</span></div>}
                  {!completed && !item.invoice && !analyzing ? <div className="rv-processing-note"><p>{mode === "demo" && !item.sample ? "Enter the invoice details below. Automatic analysis is not available for uploads in the demo." : runFailed ? "We couldn’t read the document. Try again or enter the details yourself." : "Fill in the details automatically, or enter them yourself."}</p>{mode === "live" || item.sample ? <button type="button" className="button button-small" onClick={onAnalyze}><RotateCw size={14} />{runFailed ? "Try again" : "Read document"}</button> : null}</div> : null}
                  ...
                  <p className="rv-confidence-note">Enter shipping and discounts only if they are not already included in the subtotal. Leave them blank if they do not apply.</p>
                  <p className="rv-confidence-note"><ShieldCheck size={13} />Check highlighted values against the original before saving.</p>
                </section>

                <section role="tabpanel" id="rv-panel-lines" aria-labelledby="rv-tab-lines" hidden={tab !== "lines"}>
                  <div className="rv-lines-heading"><div><h3>Line items</h3></div><button type="button" className="button button-small" onClick={() => onDraftChange({ ...draft, values: { ...values, lines: [...values.lines, emptyLine(crypto.randomUUID())] } })}><Plus size={14} />Add line</button></div>
                  {!values.lines.length ? <div className="rv-lines-empty"><div><Layers3 size={26} /></div><h3>No line items yet</h3><p>Add items from the original document.</p><button type="button" className="button" onClick={() => onDraftChange({ ...draft, values: { ...values, lines: [emptyLine(crypto.randomUUID())] } })}><Plus size={15} />Add first line</button></div> : <div className="rv-lines-list">{values.lines.map((line, index) => <div className="rv-line" key={line.id}><div className="rv-line-heading"><span>Item {index + 1}</span><button type="button" className="icon-button" onClick={() => onDraftChange({ ...draft, values: { ...values, lines: values.lines.filter((_, lineIndex) => lineIndex !== index) } })} aria-label={`Remove line ${index + 1}`}><Trash2 size={15} /></button></div><div className="rv-fields">{renderLineField(line, index, "description", "Description", true)}{renderLineField(line, index, "quantity", "Quantity")}{renderLineField(line, index, "unitPrice", "Unit price")}{renderLineField(line, index, "lineAmount", "Line amount", true)}</div><details className="rv-line-extra"><summary>Tax and unit details</summary><div className="rv-fields">{renderLineField(line, index, "unitOfMeasure", "Unit of measure")}{renderLineField(line, index, "taxRate", "Tax rate (%)")}{renderLineField(line, index, "taxAmount", "Tax amount")}</div></details></div>)}</div>}
                ...
                <section role="tabpanel" id="rv-panel-history" aria-labelledby="rv-tab-history" hidden={tab !== "history"}>
                  <div className="rv-lines-heading"><div><h3>Document history</h3></div>{!item.invoice ? <button type="button" className="button button-small" onClick={onAnalyze} disabled={analyzing}>{analyzing ? <LoaderCircle className="rv-spin" size={14} /> : <RotateCw size={14} />}{analyzing ? item.latestRun?.status === "Pending" ? "Waiting…" : "Reading…" : "Run again"}</button> : null}</div>
                  <ol className="rv-history"><li><div className="rv-history-dot"><FileText size={14} /></div><div><div className="rv-history-title"><strong>Document uploaded</strong><time>{historyTime(item.document.uploadedAt)}</time></div><p>{item.document.originalFileName}</p></div></li>{[...runs].sort((first, second) => first.startedAt.localeCompare(second.startedAt)).map((run) => <li key={run.id}><div className={`rv-history-dot ${run.status === "Failed" ? "rv-history-failed" : run.status === "Completed" ? "rv-history-complete" : ""}`}>{run.status === "Completed" ? <Check size={14} /> : run.status === "Failed" ? <CircleAlert size={14} /> : <Clock3 size={14} />}</div><div><div className="rv-history-title"><strong>{run.status === "Completed" ? "Details ready" : analysisLabel(run)}</strong><time>{historyTime(run.completedAt || run.startedAt)}</time></div>{run.status === "Failed" ? <p className="rv-history-error">{analysisFailureMessage(run)}</p> : null}</div></li>)}{item.invoice ? <li><div className="rv-history-dot rv-history-complete"><Save size={14} /></div><div><div className="rv-history-title"><strong>Invoice saved</strong><time>{historyTime(item.invoice.updatedAt)}</time></div><p>Invoice {item.invoice.invoiceNumber}</p></div></li> : null}</ol>
                </section>
              </div>
