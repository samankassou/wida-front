            ...
            <fieldset disabled={saving} className="rv-fieldset">
              <div className="rv-form-content">
                ...
                <section role="tabpanel" id="rv-panel-details" aria-labelledby="rv-tab-details" hidden={tab !== "details"}>
                  ...
                  {!completed && !item.invoice && !analyzing ? <div className="rv-processing-note"><p>{mode === "demo" && !item.sample ? "This original is stored in your browser. Enter the invoice details below; automatic extraction is available with a connected API." : runFailed ? "Extraction failed. Try again, or enter the invoice details manually." : "Run extraction to fill the invoice fields, or enter them manually."}</p>{mode === "live" || item.sample ? <button type="button" className="button button-small" onClick={onAnalyze}><RotateCw size={14} />{runFailed ? "Retry extraction" : "Run extraction"}</button> : null}</div> : null}
